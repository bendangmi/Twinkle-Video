...
- [测试] 同步首页 E2E 断言，覆盖无装饰首屏和桌面、移动端横向溢出检查。
- [部署] 基于官方 `v0.0.7` 发布二开镜像 `v0.0.7.custom.3`。

## v0.0.7.custom.2

- [模型] 将 Twinkle Model 个人凭证配置收敛到全局默认密钥名称，渠道仅保留模型、协议和系统密钥配置。
- [模型] 严格隔离 Twinkle Model 个人计费与 Twinkle Video 系统计费，并在逻辑路由、任务和生成尝试中持久化凭证模式。
- [稳定性] 个人密钥调用不再污染共享渠道健康状态、冷却状态或其他用户的系统渠道请求。
- [后台] 模型同步不再读取用户个人 Twinkle 凭证，管理界面和部署文档同步更新。
- [部署] 基于官方 `v0.0.7` 发布二开镜像 `v0.0.7.custom.2`。

## v0.0.7.custom.1

- [支付] 接入易支付，支持标准 MD5 签名的二维码/API 下单、跳转收银台、回调验签、交易查询和退款。
- [支付] 支持易支付同步回跳参数、主动交易回查、二维码页手动确认与 GET/POST 回调，支付成功后可靠恢复订单权益。
- [模型] 增加 Twinkle Model 个人账号绑定、动态密钥解析与 Twinkle Video 回退路由。
- [模型] 修复 Twinkle Model 渠道在同步稳定 `template_id` 前无法保存草稿的问题，并固定使用官方视频模型目录。
- [部署] 增加独立 `deploy` 镜像打包与外部 PostgreSQL Compose 部署方案。
- [部署] 二开镜像版本改为“官方版本 + `custom.N`”，本次基于官方 `v0.0.7` 发布 `v0.0.7.custom.1`。
- [开发] 支持前端、后端分离启动，并补充本地开发端口配置教程。
