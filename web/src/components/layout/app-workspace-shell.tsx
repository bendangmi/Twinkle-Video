...
export function AppWorkspaceShell({ children }: { children: ReactNode }) {
    ...
    return (
        <div className="workspace-shell flex h-dvh min-h-0 overflow-hidden bg-transparent text-foreground">
            <AppSidebar activeToolSlug={tool?.slug} expanded={sidebarExpanded} />
            <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
                <header className="workspace-header flex h-16 shrink-0 items-center justify-between gap-3 border-b border-[#eaecf0] bg-white/96 px-3 backdrop-blur-xl sm:px-4 lg:px-7 dark:border-[#292d33] dark:bg-[#111316]/95">
                ...
                <div className="workspace-main min-h-0 min-w-0 flex-1 overflow-hidden bg-transparent">{children}</div>
            </div>
